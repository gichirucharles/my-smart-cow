...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Calculate milk production for today
      const milkProductions = JSON.parse(localStorage.getItem("milkProductions") || "[]")
      const todayProductions = milkProductions.filter((p) => p.date === todayStr)
      const totalMilk = todayProductions.reduce((sum, p) => sum + p.amount, 0)
                ...
                <div key={index} className="flex justify-between border-b pb-2">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm text-gray-600">
                      {Math.ceil((new Date(cow.expectedDeliveryDate) - new Date()) / (1000 * 60 * 60 * 24))} days left
                    </p>
                  </div>
                </div>
              ))}
