...
-- Fixed SQL schema without generated columns to avoid immutable function errors
...
-- Milk production records (removed generated column)
CREATE TABLE IF NOT EXISTS milk_production (
  ...
  total_amount DECIMAL(8,2) DEFAULT 0, -- Will be calculated in application code
...
CREATE TABLE IF NOT EXISTS cow_feeding (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  cow_id UUID REFERENCES cows(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_cow_feeding_date ON cow_feeding(user_id, date);
...
CREATE INDEX IF NOT EXISTS idx_farm_activities_date ON farm_activities(user_id, date);
...
-- Function to automatically calculate total milk production
...
-- Trigger to automatically calculate milk production total
CREATE TRIGGER update_milk_production_total_trigger 
  BEFORE INSERT OR UPDATE ON milk_production 
  FOR EACH ROW EXECUTE FUNCTION update_milk_production_total();
